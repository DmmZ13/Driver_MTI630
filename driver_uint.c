...
typedef struct {
    ...
    uint8_t data[256]; // Buffer para armazenar os dados associados ao ID
...
int main() {
    ...
    while (pos < messageLength-1) {
        ...
        // Copia os dados para o buffer da entrada correspondente
    ...
    for (int i = 0; i < numKnownIDs; ++i) {
        printf("ID 0x%04X ", knownIDs[i].id);
        printf("%s: ", knownIDs[i].idName);
        for (int j = 0; j < knownIDs[i].length; ++j) {
            printf("%02X ", knownIDs[i].data[j]);
        }
        printf("\n");
    }
